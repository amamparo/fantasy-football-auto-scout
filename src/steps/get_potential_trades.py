...
def get_potential_trades(owners: List[Owner], players: List[Player]) -> List[Trade]:
    ...
    return [t for t in potential_trades if t.is_mutually_beneficial_in_the_short_term()]


def __evaluate_short_term_impact_of_trade(roster: List[Player], player_out: Player, player_in: Player) -> int:
    roster_after_trade = [p for p in roster if p.id != player_out.id] + [player_in]
    ...
    for week in weeks:
        ...
        if has_flex:
            starting_lineup.append(next(
                ...
                if p.position in ['WR', 'RB', 'TE'] and \
                p.id not in map(lambda x: x.id, starting_lineup)
            ))
